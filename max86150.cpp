...
bool MAX86150::begin(uint8_t i2c_address, I2C_HandleTypeDef *i2c_handle) {
	i2c_han = i2c_handle;
...
uint8_t MAX86150::getINT1(void) {
	return (readRegister8(i2c_addr, MAX86150_INTSTAT1));
}
uint8_t MAX86150::getINT2(void) {
	return (readRegister8(i2c_addr, MAX86150_INTSTAT2));
}

void MAX86150::enableAFULL(void) {
	bitMask(MAX86150_INTENABLE1, MAX86150_INT_A_FULL_MASK,
...
void MAX86150::enableDATARDY(void) {
	bitMask(MAX86150_INTENABLE1, MAX86150_INT_DATA_RDY_MASK,
...
void MAX86150::enableALCOVF(void) {
	bitMask(MAX86150_INTENABLE1, MAX86150_INT_ALC_OVF_MASK,
...
void MAX86150::enablePROXINT(void) {
	bitMask(MAX86150_INTENABLE1, MAX86150_INT_PROX_INT_MASK,
...
void MAX86150::softReset(void) {
	...
	while (millis() - startTime < 100) {
		uint8_t response = readRegister8(i2c_addr, MAX86150_SYSCONTROL);
...
void MAX86150::setPulseAmplitudeRed(uint8_t amplitude) {
	writeRegister8(i2c_addr, MAX86150_LED2_PULSEAMP, amplitude);
}

void MAX86150::setPulseAmplitudeIR(uint8_t amplitude) {
	writeRegister8(i2c_addr, MAX86150_LED1_PULSEAMP, amplitude);
}

void MAX86150::setPulseAmplitudeProximity(uint8_t amplitude) {
	writeRegister8(i2c_addr, MAX86150_LED_PROX_AMP, amplitude);
}

void MAX86150::setProximityThreshold(uint8_t threshMSB) {
	// The threshMSB signifies only the 8 most significant-bits of the ADC count.
	writeRegister8(i2c_addr, MAX86150_PROXINTTHRESH, threshMSB);
}
...
void MAX86150::disableSlots(void) {
	writeRegister8(i2c_addr, MAX86150_FIFOCONTROL1, 0);
	writeRegister8(i2c_addr, MAX86150_FIFOCONTROL2, 0);
}
...
void MAX86150::enableFIFORollover(void) {
	bitMask(MAX86150_FIFOCONFIG, MAX86150_ROLLOVER_MASK,
...
uint8_t MAX86150::getWritePointer(void) {
	return (readRegister8(i2c_addr, MAX86150_FIFOWRITEPTR));
}
...
uint8_t MAX86150::readPartID() {
	return readRegister8(i2c_addr, MAX86150_PARTID);
}
...
uint16_t MAX86150::check(void) {
	...
	if ( (readPointer != writePointer) || (ovfCounter != 0)) {
		...
		uint8_t txData = MAX86150_FIFODATA;
		write(&txData, 1);
		...
		while (bytesLeftToRead > 0) {
			...
			read(data, toGet); // note: can increase I2C_BUFFER_LENGTH for stm32 platform but leaving at current definition for now
			index = 0; // reset index
...
void MAX86150::bitMask(uint8_t reg, uint8_t mask, uint8_t thing) {
	...
	uint8_t originalContents = readRegister8(i2c_addr, reg);

	// Zero-out the portions of the register we're interested in
	originalContents = originalContents & mask;

	// Change contents
	writeRegister8(i2c_addr, reg, originalContents | thing);
}

uint8_t MAX86150::readRegister8(uint8_t address, uint8_t reg) {
	uint8_t data;
	if (HAL_OK
			== HAL_I2C_Mem_Read(i2c_han, address, reg, 1, &data, 1, 10)) {
		return data;
	} else {
		return 0;
	}
...
bool MAX86150::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
	if (HAL_OK
			== HAL_I2C_Mem_Write(i2c_han, address, reg, 1, &value, 1, 10)) {
		return true;
	} else {
		return false;
	}
...
bool MAX86150::write(uint8_t *data, uint8_t size) {
	if (HAL_OK
